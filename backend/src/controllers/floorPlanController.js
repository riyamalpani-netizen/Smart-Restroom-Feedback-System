...
async function createFloorPlan(req, res) {
  try {
    const { floorId, name, imageData, width, height, posX, posY, rotation, scale, geoBounds, geoJsonData } = req.body;
...
async function updateFloorPlan(req, res) {
  try {
    const { id } = req.params;
    const { name, imageData, width, height, posX, posY, rotation, scale, geoBounds, geoJsonData } = req.body;
    const userRole = req.user?.role;
    const userOrgId = req.user?.organizationId;
